...
getReqPromise("patient/family/isAssign",{}).then(function(data) {
	...
	if(assign==1) {
		//已分拣+已签约
		window.location.href = "../html/doctor-homepage.html?teamCode="+data.data.teamCode+"&orgName="+data.data.orgName;
	} else if(assign == 0){
		//已分拣+未签约
		window.location.href = "../html/search-team.html?hospital="+data.data.unitCode+"&name="+data.data.unitName;
	}else{
		//未分拣
