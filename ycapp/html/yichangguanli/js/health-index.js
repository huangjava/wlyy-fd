...
function addHealthIndex(intervene, operTime, value1, value2, value3, value4, value5, value6, value7, type, successFunction) {
	...
	dd.showModal();
	//发送ajax请求
//	sendPost("patient/health_index/add", params, "json", "post", addHealthIndexFailed, successFunction);
}
...
function addBloodPressureSuccess(res) {
	if (res.status == 200) {
		dd.close();
		window.location.href='health-record.html?charType=2';
	} else {
		//非200则为失败
...
function addBloodSugarSuccess(res) {
	if (res.status == 200) {
		...
		window.location.href='health-record.html?charType=1';
		dd.close();
	} else {
		//非200则为失败
		addHealthIndexFailed(res);
...
function addWaistlineSuccess(res) {
	if (res.status == 200) {
		dd.close();
		window.location.href='health-record.html?charType=4';
	} else {
		//非200则为失败
		addHealthIndexFailed(res);
	}
...
function addWeightSuccess(res) {
	if (res.status == 200) {
		dd.close();
		window.location.href='health-record.html?charType=3';
	} else {
		//非200则为失败
		addHealthIndexFailed(res);
	}
