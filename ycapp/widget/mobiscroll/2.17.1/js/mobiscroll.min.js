(function($, undefined) {
		...
		util: {
			...
			addIcon: function($control, ic) {
				...
				if (icon) {
					...
					extend(icons, ic);
					$parent.addClass((icons.right ? 'mbsc-ic-right ' : '') + (icons.left ? ' mbsc-ic-left' : '')).find('.mbsc-input-wrap').append(icons.left ? '<span class="mbsc-input-ic mbsc-left-ic mbsc-ic mbsc-ic-' + icons.left + '"></span>' : '').append(icons.right ? '<span class="mbsc-input-ic mbsc-right-ic mbsc-ic mbsc-ic-' + icons.right + '"></span>' : '')
				}
			},
	...
	$.mobiscroll.classes.Base = function(el, settings) {
		...
		that.tap = function(el, handler, prevent) {
			var startX, startY, target, moved;
			...
			if (s.tap) {
				el.on('touchstart.dw pointerdown.dw', onStart).on('touchcancel.dw pointercancel.dw', onCancel).on('touchmove.dw', onMove).on('touchend.dw', onEnd)
			}
...
(function($, window, document, undefined) {
	...
	ms.classes.Frame = function(el, settings, inherit) {
		...
		that.show = function(prevAnim, prevFocus) {
			...
			$markup.on('touchstart mousedown pointerdown', '.dwb-e', onBtnStart).on('touchend', '.dwb-e', onBtnEnd);
...
(function($, window, document, undefined) {
	...
	classes.Scroller = function(el, settings, inherit) {
		var $markup, btn, isScrollable, itemHeight, multiple, pixels, s, scrollDebounce, trigger, click, moved, start, startTime, stop, p, min, max, target, index, lines, timer, that = this,
			$elm = $(el),
			iv = {},
			pos = {},
			wheels = [];
		...
		function generateWheelItems(i) {
			...
			$.each(values, function(j, v) {
				...
				html += '<div role="option" aria-selected="false" class="dw-li dw-v" data-val="' + keysHtml + '"' + (labels[j] ? ' aria-label="' + labels[j] + '"' : '') + ' style="height:' + itemHeight + 'px;line-height:' + itemHeight + 'px;">' + '<div class="dw-i"' + (lines > 1 ? ' style="line-height:' + Math.round(itemHeight / lines) + 'px;font-size:' + Math.round(itemHeight / lines * 0.8) + 'px;"' : '') + '>' + v + '</div></div>';
				l++
			});
			html += '</div>';
			return html
		...
		that._attachEvents = function($markup) {
			$markup.on('keydown', '.dwwl', onKeyDown).on('keyup', '.dwwl', onKeyUp).on('touchstart mousedown', '.dwwl', onStart).on('touchmove', '.dwwl', onMove).on('touchend', '.dwwl', onEnd).on('touchstart mousedown', '.dwwb', onBtnStart).on('touchend touchcancel', '.dwwb', onBtnEnd);
			if (s.mousewheel) {
...
(function($, undefined) {
	var ms = $.mobiscroll,
		...
		preset = function(inst) {
			...
			var i, k, keys, values, wg, start, end, hasTime, mins, maxs, orig = $.extend({}, inst.settings),
			...
			function validateTimes(vobj, i, v, temp, y, m, d, target, valid) {
				var dd, ss, str, parts1, parts2, prop1, prop2, v1, v2, j, i1, i2, add, remove, all, hours1, hours2, hours3, spec = {},
					steps = {
						h: stepH,
						i: stepM,
						s: stepS,
...
(function($, undefined) {
	...
	ms.presets.scroller.select = function(inst) {
		var change, defaultValue, group, groupArray, groupChanged, groupTap, groupWheelIdx, i, input, optionArray, optionWheelIdx, option, origValues, prevGroup, timer, batchChanged = {},
			batchStart = {},
			batchEnd = {},
			tempBatchStart = {},
			tempBatchEnd = {},
...
(function($, undefined) {
	...
	ms.presets.scroller.list = function(inst) {
		var orig = $.extend({}, inst.settings),
			...
			origReadOnly = s.readonly,
			elm = $(this),
			input, prevent, id = this.id + '_dummy',
			lvl = 0,
			ilvl = 0,
			timer = {},
			currLevel, currWheelVector = [],
